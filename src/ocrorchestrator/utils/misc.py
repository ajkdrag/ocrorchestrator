...
def generate_dynamic_model(
    ...
    for field_info in fields:
        ...
        if field.startswith("list_"):
            field_definitions[field] = (list, Field(default=[], description=desc))
        elif field.startswith("is_"):
            field_definitions[field] = (bool, Field(default=False, description=desc))
        elif field.startswith("num_"):
            field_definitions[field] = (int, Field(default=None, description=desc))
        elif field.endswith(("_no", "_date", "_name")):
            field_definitions[field] = (str, Field(default="", description=desc))
        elif field.endswith("amount"):
            field_definitions[field] = (float, Field(default=None, description=desc))
        elif field.endswith("amounts"):
            field_definitions[field] = (dict, Field(default={}, description=desc))
        else:
            field_definitions[field] = (str, Field(default="", description=desc))
